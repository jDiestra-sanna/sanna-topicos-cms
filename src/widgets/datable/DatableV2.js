...
class Datable extends Component {
    ...
    constructor(props, context) {
        ...
        if (!this.state.fil.page) {
            this.defaultFil.page = 0;
        }
    ...
    handleRequestSort(col, val) {
        ...
        this.setState({
            fil: {
                ...this.state.fil,
                order_dir: this.state.fil.order_col === order_col && this.state.fil.order_dir === 'DESC' ? 'ASC' : 'DESC',
                order_col: order_col
            }
        }, this.loadData);
    }
    ...
    onChangeRowsPerPage = (event) => {
        ...
        this.setState({
            fil: {...this.state.fil, page: 0, limit}
        }, this.loadData);
    ...
    goRemove(id) {
        Alert.confirm('¿Seguro que quieres eliminarlo?', 'Eliminar', async () => {
            const response = await Api.delete(`${this.endpoint}/${id}`, {}, 'Eliminando...');
    ...
    goEnable(id) {
        Alert.confirm('¿Seguro que quieres activarlo?', 'Activar', async () => {
            const response = await Api.patch(`${this.endpoint}/${id}/state`, { state: this.props.enabledState }, 'Activando...');
    ...
    goDisable(id) {
        Alert.confirm('¿Seguro que quieres desactivarlo?', 'Desactivar', async () => {
            const response = await Api.patch(`${this.endpoint}/${id}/state`, { state: this.props.disabledState }, 'Desactivando...');
    ...
    mkHeadLabelCell(key, val) {
        return (typeof val.sort == 'undefined' || val.sort) ? (
            <TableSortLabel
                ...
                onClick={() => this.handleRequestSort(key, val)}>
                {val.value}
            </TableSortLabel>
        ) : <span>{val.value}</span>;
    }
